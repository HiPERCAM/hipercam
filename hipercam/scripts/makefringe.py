...
def makefringe(args=None):
    ...
    At long wavelengths, CCDs suffer what is known as "fringing", which
    looks something like the coloured patterns you see when there is a
    thin layer of oil on water. Both patterns are caused by interference.
    In the case of CCDs this is caused by sky emission lines and is variable
    in strength, and additive in nature.
    ...
    (optionally), calculates a median count level of each one which is
    subtracted and divide into each frame indivually. The
    pixel-by-pixel median of all frames is then calculated. Finally an
    optional amount of smoothing can be applied on the basis that it
    should have a smaller scale than any of the fringing.
