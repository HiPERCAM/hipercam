...
def logsearch(args=None):
    ...
    .. Note::
       ...
       depending upon your network speed. The downloads are stored in
       $HOME/.hipercam/dbases [or $HIPERCAM_ENV/dbases if you have set
       HIPERCAM_ENV]. The databases on the Warwick server are on the
       password-protected log pages. Thus the first time you use
       logsearch, you will be asked for these, one each for the three
       cameras. The program will store these passwords in your system
       "keyring" in a folder called "Data logs" (look up the python
       module "keyring" to find out more). If the passwords change
       (not often I hope), you will have to delete those stored in
       your keyring and re-enter them.
