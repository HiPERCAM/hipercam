...
def splice(args=None):
    ...
    with Cline('HIPERCAM_ENV', '.hipercam', command, args) as cl:
        ...
        infile1 = cl.get_value(
            ...
            cline.Fname('frame1', hcam.HCAM)
            )
        mccd1 = hcam.MCCD.read(infile1)

        infile2 = cl.get_value(
            'input2', 'second input frame',
            cline.Fname('frame2', hcam.HCAM)
            )
        mccd2 = hcam.MCCD.read(infile2)

        if len(mccd1) > 1:
            ccd = cl.get_value(
                'ccd', "CCD(s) to transfer ('0' for all)", '1'
                )
            if ccd == '0':
        ...
        if len(wins) == 0:
            ...
            win = cl.get_value(
                'win', "windows to transfer ('0' for all)", '0'
                )
            if win != '0':
                wins = wins.split()
        ...
        outfile = cl.get_value(
            'output', 'output file',
            cline.Fname('output', hcam.HCAM, cline.Fname.NEW)
            )
