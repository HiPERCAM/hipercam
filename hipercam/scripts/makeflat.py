...
def makeflat(args=None):
    ...
    (optionally), and calculates the mean count level in each CCD, normalises
    by the mean and writes out the results to temporary files. For each CCD it
    then sorts the files by mean level, and for those that lie between defined
    limits it takes the median of the frames in groups of defined size. Thus,
    say one had 75 OK images, then these would be divided into 10 groups, the
    first 9 having 7 frames, the last having 16. The median average of each of
    these would be taken. In each case the mean levels would be adjusted to be
    the same before taking the average to overcome the problem of taking a
    median of a time-variable sky. The assumption is that while the level may
    vary, the pattern of the image does not. It is up to the user to check
    that this is correct. Each of the medians is adjusted to have a mean equal
    to the sum of the means of the input frames. Finally the normal average of
    all of these median frames is taken and the mean level normalised to
    1. The first step, taking the median in groups is designed to remove the
    stars assuming that the telescope was spiralled. The size of the groups
    ('ngroup' below is a crucial parameter in whether this works). A good
    strategy is to run makeflat for a succession of ever larger 'ngroup'
    and then to divide the results into each other to see if stars are
    visible.

    The final step, the average of the medians with adjusted mean levels, is
    to ensure that the flats are combined in a way that reflects the level of
    signal that they have, to avoid giving equal weight to the median of a
    series of flats with 20,000 counts per pixel and another series with 1000
    counts per pixel. This somewhat complex procedure is implemented through a
    series of temporary files which are written and read as the script runs,
    but deleted at its end. This allows very large numbers to be combined as
    long as there is enough memory to load 'ngroup' CCDs simultaneously which
    ...
    Arguments::
        ...
        ngroup  : int
           the number of frames. Probably should be at least 5, preferably more. Experiment
           to see its effect.
