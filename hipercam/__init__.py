...
it into individual files containing the 5 CCDs, etc. It provides a combination
of an API to access HiPERCAM data and high-level scripts to enact certain
commonly-required operations in reduction. Most users will be most interested
in the latter, and if that is the case for you, your first port of entry is
probably to look at the 'scripts' subpackage, e.g. pydoc hipercam.scripts.
...
thing. This is trus for most of the sub-modules, so looking through the
...
sub-module hipercam.cline, and the plotting sub-sections, 'mpl' and 'pgp'.
