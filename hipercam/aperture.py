...
class Aperture(object):
    """
    Class representing a photometric aperture for measuring the flux of
    a star. At its most basic this consists of 3 circles representing
    the object, radius `r1`, and the sky annulus between radii `r2` and
    `r3`, centered at a specific position, but there are several extra
    ...
    def __init__(self, x, y, r1, r2, r3, ref, link=None, mask=None, extra=None):
        ...
        if link is not None and not isinstance(link,list):
            raise HipercamError('Aperture.__init__: link must be a list')

        if mask is not None and not isinstance(mask,list):
            raise HipercamError('Aperture.__init__: mask must be a list')

        if extra is not None and not isinstance(extra,list):
            raise HipercamError('Aperture.__init__: extra must be a list')

    def __repr__(self):
        return 'Aperture(x=' + repr(self.x) + ', y=' + repr(self.y) + \
            ', r1=' + repr(self.r1) + ', r2=' + repr(self.r2) + \
            ', r3=' + repr(self.r3) + ', ref=' + repr(self.ref) + \
            ', link=' + repr(self.link) + ', mask=' + repr(self.mask) + \
            ', extra=' + repr(self.extra) + ')'

    def clash(self, aper):
        """
        Dummy routine to allow `Aperture` objects to be inserted into
        `Group`s. There are no conditions under which teo `Aperture`
